...
summaries << '【落差ランキング】'
summaries << '[差が小さい]'
summaries << target_diffs.map { |diff| ajust_space("+#{diff}(#{reward + diff}位)との差", 27) }.join
summaries += cliff_rankings.values.map do |ranking|
  ranking[0...10].map.with_index(1) do |record, rank|
    ajust_space("#{'%02d' % rank}位 #{record[:idol].name.to_s.ljust(5, '　')}: #{readable_unit(record[:diff])}", 27)
  end
end.transpose.map(&:join)
summaries << ''
